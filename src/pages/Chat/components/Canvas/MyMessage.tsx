...
const MyMessage: React.FC<Props> = ({message, alterHeader, selectMessage}) => {
	...
	// validate isSelect and imageModal
	useEffect(() => {
		if (isSelect && imageModal) {
			setIsSelect(false);
			selectMessage(message._id);
		}
	}, [isSelect, imageModal, selectMessage, message]);
	...
	return (
		...
		>
			...
			{message.image.url && !message.image.caption && (
				<Card className={classes.message}>
					<Card className={classes.imageWrapper} onClick={(): void => setImageModal(true)}>

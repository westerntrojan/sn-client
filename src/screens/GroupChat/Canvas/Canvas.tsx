...
const Canvas: React.FC<Props> = ({messages, scrollDown, loading}) => {
	...
	const {handleRemoveMessages} = useContext(GroupChatContext);
		...
		<Paper className={classes.root}>
			<CanvasContext.Provider value={{state, dispatch}}>
				...
				<div className={classes.messages} ref={messagesContainerRef}>
							...
							if (message.user._id === auth.user._id) {
								return (
									<MyMessage
										key={message._id}
										message={message}
										alterHeader={Boolean(state.selectedMessages.length)}
										selectMessage={() => {
											dispatch({
					...
					{!auth.user && messages.map(message => <Message key={message._id} message={message} />)}
				</div>
